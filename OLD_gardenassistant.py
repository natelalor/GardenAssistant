...
def optimize_garden(dimensions_list, veggies_list):
    ...
    for veggie in veggie_info: 
        ...
        temp_veggie_list.append(width_row_information) # WIDTH PER ROW (LIST)
...
def fill_the_garden(sorted_complete_veggie_information, length_of_garden, width_of_garden):
    ...
    iterator = 0
    current_length = length_of_garden

    plant_subset = []
    for veggie in sorted_complete_veggie_information:
        plant_subset.append(veggie[1])

    # add 1 row of everything, minus from current length
        


    # if current length - biggest plant > 0:
    # if (current_length - ) > 0:
        # you can plant another row of the biggest plant
